                        				...
                        				<table class="table table-bordered">
                                            ...
                                            @foreach($article_comments as $key => $comment)
                                              <tr>
                                                ...
                                                <td>{{ $comment->userArticleComment->name }}</td>
                                                ...
                                                <td>
                                                  ...
                                                  <td style="display: flex;">
                                                      @if($comment->status == 1)
                                                          <a href="{{ route('admin.article-no-approve-status' , ['comment' => $comment->id]) }}" class="btn btn-success btn-sm">تایید شده</a>
                                                      @else
                                                          <a href="{{ route('admin.article-approve-status' , ['comment' => $comment->id]) }}" class="btn btn-warning btn-sm">در انتظار تایید</a>
                                                      @endif
                                                          <a href="{{ route('admin.article-reply-comment-post' , ['comment' => $comment->id]) }}" style="margin-right: 10px;" class="btn btn-warning btn-sm">پاسخ دادن</a>
                                                          <a href="{{ route('admin.article.comments.edit' , ['comment' => $comment->id]) }}" style="margin-right: 10px;" class="btn btn-primary btn-sm">ویرایش</a>

                                                          <form action="{{ route('admin.article.comments.delete', ['comment' => $comment->id]) }}" method="POST">
                                                          @csrf
                                                          @method('DELETE')
